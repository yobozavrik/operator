...
async function main() {
    try {
        const query = "SELECT n.nspname as schema, p.proname as name FROM pg_proc p JOIN pg_namespace n ON p.pronamespace = n.oid WHERE p.prosrc ILIKE '%EXECUTE%' AND n.nspname = 'public'";
        ...
        console.log('Functions with EXECUTE in public schema:');
