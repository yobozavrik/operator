...
async function main() {
    try {
        console.log('--- Aggregated metrics for 2026-02-24 ---');
        const query = "SELECT sum(qty_delivered) as delivered, sum(qty_fresh_sold + qty_disc_sold) as sold, sum(qty_waste) as waste FROM bakery1.mv_craft_daily_mart WHERE date = '2026-02-24'";
