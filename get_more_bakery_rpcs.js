...
async function main() {
    try {
        const functions = ['f_craft_get_network_metrics', 'f_craft_get_store_ranking', 'f_craft_get_sku_cards'];
        for (const fn of functions) {
            console.log(`--- ${fn} ---`);
            const query = `SELECT pg_get_functiondef(p.oid) as definition FROM pg_proc p JOIN pg_namespace n ON p.pronamespace = n.oid WHERE n.nspname = 'bakery1' AND p.proname = '${fn}'`;
