-- Виправляємо розподіл: віднімаємо випічку/ліпку поточного дня від залишку!
-- Це вирішує проблему, коли виробництво "падає" на той самий склад (storage_id=2 Гравітон) і збиває розрахунок власного дефіциту.
...
CREATE OR REPLACE VIEW graviton.distribution_base AS
 WITH mapping AS (
         ...
         -- Рахуємо що вироблено СЬОГОДНІ на кожному складі Гравітону
         SELECT 
            m.storage_id,
            mi.product_id,
         ...
         GROUP BY m.storage_id, mi.product_id
        )
 SELECT m.spot_id AS "код_магазину",
    ...
    cat.category_name,
    -- ВІДНІМАЄМО СЬОГОДНІШНЄ ВИРОБНИЦТВО ВІД ПОТОЧНОГО ЗАЛИШКУ
    GREATEST((0)::numeric, COALESCE(st.current_stock, (0)::numeric) - COALESCE(tp.produced_today, (0)::numeric)) AS current_stock,
    COALESCE(sl.avg_sales_day, (0)::numeric) AS avg_sales_day,
    ceil((COALESCE(sl.avg_sales_day, (0)::numeric) * (3)::numeric)) AS min_stock,
    ceil(GREATEST((0)::numeric, ((COALESCE(sl.avg_sales_day, (0)::numeric) * (3)::numeric) - GREATEST((0)::numeric, COALESCE(st.current_stock, (0)::numeric) - COALESCE(tp.produced_today, (0)::numeric))))) AS deficit_kg
   FROM (((mapping m
     ...
     -- ПРИЄДНУЄМО СЬОГОДНІШНЄ ВИРОБНИЦТВО
     LEFT JOIN today_production tp ON (m.storage_id = tp.storage_id AND cat.product_id = tp.product_id)
     )
...
CREATE OR REPLACE VIEW graviton.distribution_base_test AS
 ...
 SELECT m.spot_id AS "код_магазину",
    ...
    ceil(GREATEST((0)::numeric, ((COALESCE(sl.avg_sales_day, (0)::numeric) * (4)::numeric) - GREATEST((0)::numeric, COALESCE(st.current_stock, (0)::numeric) - COALESCE(tp.produced_today, (0)::numeric))))) AS deficit_kg
   FROM (((mapping m
     CROSS JOIN catalog cat)
     LEFT JOIN stock_data st ON (((m.storage_id = st.storage_id) AND (lower(TRIM(BOTH FROM cat.product_name)) = lower(TRIM(BOTH FROM st.ingredient_name))))))
     LEFT JOIN sales_data sl ON (((m.spot_id = sl.spot_id) AND (cat.product_id = sl.product_id)))
