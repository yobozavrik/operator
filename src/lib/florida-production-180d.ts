...
export async function fetchFloridaProduction180dRows(
    ...
    const tableSelect =
        'product_id, product_name, total_qty_180d, prod_days, avg_qty_per_prod_day, avg_qty_per_calendar_day, min_day_qty, max_day_qty, last_manufacture_at, network_min_stock, network_avg_sales_day, network_stock_now, shops_count, source_storage_id, refreshed_at, updated_at';
...
export async function fetchFloridaProduction180dProductIds(
    ...
    let liveTodayIds: number[] = [];

    try {
        const liveRows = (await getTodayManufactures({
            categoryKeywords: null,
            storageId: FLORIDA_WORKSHOP_STORAGE_ID,
        })) as Array<Record<string, unknown>>;

        liveTodayIds = liveRows
            .map((row) => Number(row.product_id))
            .filter((id) => Number.isFinite(id) && id > 0);
    } catch {
        // Non-blocking: if Poster is unavailable, we still return DB-based 180d set.
    }
