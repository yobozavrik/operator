...
const BISparkline = ({ data }: { data: number[] }) => {
    ...
    return (
        <div className="flex items-end gap-0.5 h-6 w-24">
            {data.map((val, i) => (
                <div
                    key={i}
                    className="w-2 bg-brand-primary opacity-50 rounded-t-[1px]"
                    style={{ height: `${((val - min) / range) * 100}%`, minHeight: '1px' }}
                />
            ))}
        </div>
    );
...
export const BIProductionMatrix = ({ queue }: { queue: ProductionTask[] }) => {
    return (
        <div className="bi-panel rounded-xl overflow-hidden flex flex-col h-full border-t border-brand-primary/20">
                                ...
                                <tr key={item.id} className={cn(
                                    ...
                                    <td className="px-4 py-2">
                                        <div className="flex flex-col">
                                            ...
                                            <span className="text-[9px] font-bold text-slate-500">{item.category}</span>
                                        </div>
                                    </td>
                                    <td className="px-4 py-2 font-mono-bi text-xs text-slate-300">
                                        {item.totalStockKg.toFixed(1)} <span className="text-[10px] opacity-30">кг</span>
                                    </td>
                                    <td className="px-4 py-2">
                                        <div className="flex items-center gap-2">
                                            <TrendingUp size={12} className="text-[#58A6FF]" />
                                            <span className="text-xs font-mono-bi font-black text-slate-300">
                                    ...
                                    <td className="px-4 py-2">
                                        <div className={cn(
                                            "inline-flex items-center px-2 py-0.5 rounded text-[10px] font-black font-mono-bi border",
                                    ...
                                    <td className="px-4 py-2 text-right">
                                        <div className="inline-flex items-center gap-2 bg-surface-900 border border-surface-700 rounded px-2 py-1 group-hover:border-brand-primary transition-colors">
                                            <span className="w-12 text-right font-mono-bi text-xs font-black text-brand-primary">
