...
import { BarChart3, AlertTriangle, TrendingDown, Store } from 'lucide-react';
...
export const SeniorAnalytics = ({ queue }: { queue: ProductionTask[] }) => {
    ...
    // 2. Mock "Sadhora" data
    const sadhoraEmpty = queue.filter(t => t.storeStocks['Садгора'] === 0).length;

    return (
        <div className="space-y-6">
            {/* Top Deficit Bar Chart */}
            <div className="bi-panel p-6 rounded-2xl border border-white/5">
                <div className="flex items-center gap-3 mb-8">
                    ...
                    <div>
                        ...
                        <p className="text-[10px] font-bold text-slate-500 uppercase mt-0.5">Сумарний недобор у КГ</p>
                    </div>
                </div>

                <div className="space-y-5">
                    {deficitByCategory.map((d, i) => (
            ...
            <div className="bi-panel p-6 rounded-2xl border border-[#F85149]/20 bg-gradient-to-br from-[#161B22] to-[#F85149]/5">
                <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h3 className="text-sm font-black text-white uppercase tracking-widest">Садгора: Тривога</h3>
                ...
                <div className="bg-[#0D1117]/50 rounded-xl p-4 border border-[#30363D]">
                    <p className="text-[10px] font-bold text-slate-500 uppercase mb-2">Обнулені позиції</p>
                    <div className="text-3xl font-mono-bi font-black text-white mb-1">
                        {sadhoraEmpty} <span className="text-sm text-[#F85149]">SKU</span>
                    </div>
                ...
                <button className="w-full mt-4 py-3 border border-[#F85149]/30 hover:bg-[#F85149]/10 text-[#F85149] text-[10px] font-black uppercase tracking-[0.2em] rounded-lg transition-all">
                    Переглянути деталі
                </button>
            </div>
        </div>
