...
async function run() {
    ...
    const manufacturesData = await fetchPoster('storage.getManufactures', { dateFrom: dateStr, dateTo: dateStr, storage_id: '2' });
    const posterManufactures = manufacturesData.response || [];

    // EXACT UI LOGIC
    let total = 0;
    const gravitonM = posterManufactures.filter((m) => String(m.storage_id) === '2');
        ...
        if (m.products) {
            m.products.forEach((p) => {
                ...
                if (pid && catalogSet.has(pid)) {
                    total += parseFloat(p.product_num || '0');
                }
            });
        }
    });

    console.log("Total computed exact UI logic for storage 2:", total.toFixed(3));
}
run();
